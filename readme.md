...
`cde` can be [abbreviated](https://github.com/jalanb/kd/blob/v0.7.0/cd.sh#L19) to just `c`, e.g.
```shell
$ c ..; pwd
/usr/local
```

And sometimes can be [abbreviated](https://github.com/jalanb/kd/blob/v0.7.0/cd.sh#L90) away entirely, e.g.
```shell
$ c /u l b; pwd
/usr/local/bin
$ ...; pwd
...
If nothing matches then `c` [tries directories in $PATH which have matching executables](https://github.com/jalanb/kd/blob/v0.7.0/cd.py#L261). For example, this will give `/bin`:
...
When looking for partial names `c` will [look for each of these in turn](https://github.com/jalanb/kd/blob/v0.7.0/cd.py#L649), stopping as soon as it gets some match
